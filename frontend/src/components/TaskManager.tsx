...
import { Plus, Check, Clock, Calendar, Trash2, Edit3, Search, X, AlertTriangle } from 'lucide-react';
...
export const TaskManager: React.FC<Props> = ({ tasks, onTaskChange }) => {
  ...
  const openCompleteModal = (task: Task) => {
    ...
    setCompleteMinutes(task.estimated_minutes || 30);
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      await TaskService.completeTask(completeTask.id, completeMinutes);
    ...
    <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {t.due_date && (
                    <span className="flex items-center gap-1 text-[#D97706]">
                      <Calendar className="w-3 h-3" /> {formatUtcDate(t.due_date)}
                    </span>
                  )}
                </div>
      ...
      {completeTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-4xl p-6 sm:p-8 shadow-cozy border border-[#FFDFE5]">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-xl font-bold text-[#4A3E3D]">Complete Task 🌸</h3>
                <p className="text-xs text-[#9CA3AF] mt-1">'{completeTask.title}'</p>
              </div>
              <button
                onClick={() => setCompleteTask(null)}
                className="text-[#9CA3AF] hover:text-[#FF8DA1] p-1 rounded-full hover:bg-[#FAF6F0]"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <input
              ...
              onChange={(e) => setCompleteMinutes(parseInt(e.target.value, 10) || 0)}
              className="w-full px-4 py-2.5 bg-[#FAF6F0] rounded-2xl text-sm border border-[#FFDFE5] focus:ring-2 focus:ring-[#10B981] outline-none"
            />
            ...
            <div className="flex items-center justify-end gap-3 pt-6">
              ...
              <button
                ...
                className="bg-[#10B981] hover:bg-[#059669] text-white text-xs font-bold px-6 py-2.5 rounded-full shadow-cozy transition-all disabled:opacity-60"
              >
      ...
      {deleteTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-4xl p-6 sm:p-8 shadow-cozy border border-[#FECACA]">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-[#FEE2E2] flex items-center justify-center text-[#EF4444]">
                <AlertTriangle className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-lg font-bold text-[#4A3E3D]">Delete Task?</h3>
                <p className="text-xs text-[#9CA3AF]">'{deleteTask.title}' and its RAG memory will be removed.</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-4">
              ...
              <button
                ...
                className="bg-[#EF4444] hover:bg-[#DC2626] text-white text-xs font-bold px-6 py-2.5 rounded-full shadow-cozy transition-all disabled:opacity-60"
              >
